...
async function carregar() {
  dados = (await api('vendedorData', {
    vendedor: vendedorSessao.nome,
    token: vendedorSessao.token
  })).data;
...
function renderPendentes() {
  const pend = dados.mesas.filter(m =>
    m.validacao === 'pendente' && m.vendedor === vendedorSessao.nome
  );
  ...
  wrap.innerHTML = pend.map(m => `
    <div class="registro">
      <strong>
        Mesa ${padMesa(m.numero)}
        <br>
        <small>${safeText(m.comprador || '')}</small>
      </strong>
      <span class="tag pendente">pendente</span>
      <button type="button" onclick="abrirMesaVendedor(${Number(m.numero)})">Abrir</button>
    </div>
  `).join('');
}
...
async function abrirMesaVendedor(numero) {
  ...
  if (mesa.status === 'vendida') {
    hideLoading();
    return alert('Essa mesa já foi vendida. Somente o Admin pode alterar.');
  }

  if (mesa.status === 'reservada' && mesa.vendedor && mesa.vendedor !== vendedorSessao.nome) {
    hideLoading();
    return alert('Essa mesa está reservada por outro vendedor.');
  }
  ...
  el('mesaStatus').value = mesa.validacao === 'pendente'
    ? 'reservada'
    : (mesa.status === 'livre' ? 'vendida' : mesa.status);
  ...
  try {
    ...
    if (lock && lock.lockExpires) {
      pararTimer = iniciarTimerReserva(
        { lockExpires: lock.lockExpires },
        el('tempoReserva'),
        cancelarMesa
      );
    } else {
      el('tempoReserva').textContent = '';
    }
...
el('salvarMesa').onclick = async e => {
  ...
  if (status === 'vendida' && !comprovanteSelecionado && !mesa.comprovanteUrl) {
    return el('avisoMesa').textContent = 'Para vender, selecione ou mantenha um comprovante.';
  }
...
el('btnRejeitarCompra').onclick = async e => {
  ...
  if (!confirm('Rejeitar a compra e liberar a mesa?')) return;

  try {
    await api('rejeitarCompra', {
      token: vendedorSessao.token,
      vendedor: vendedorSessao.nome,
      numero: mesaAtual
    });
...
async function iniciarSessao() {
  ...
  const btnPerfil = el('btnPerfilVendedor');
  if (btnPerfil) {
    btnPerfil.title = vendedorSessao.nome;
  }
