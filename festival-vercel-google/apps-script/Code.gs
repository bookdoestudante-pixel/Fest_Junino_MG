...
const ADMIN_PASSWORD = '123456'; // Troque antes de usar
...
function getVendedores_() {
  return rows_(SHEETS.VENDEDORES)
    .filter(v => String(v.ativo || 'true') !== 'false')
    .map(v => ({ nome: String(v.nome || ''), telefone: String(v.telefone || ''), pin: String(v.pin || '') }));
}
...
function clienteComprar(body) {
  ...
  saveMesa_(m, {
    status: 'reservada', comprador: body.comprador, contato: body.contato, vendedor: body.vendedor,
    pagamento: body.pagamento, comprovanteNome: comp.nome, comprovanteUrl: comp.url,
    origem: 'cliente', validacao: 'pendente', lockOwner: '', lockExpires: ''
  });
...
function vendedorSalvarMesa(body) {
  ...
  saveMesa_(m, {
    status: body.status, comprador: body.comprador, contato: body.contato, vendedor: body.vendedor,
    pagamento: body.pagamento, comprovanteNome: comp.nome, comprovanteUrl: comp.url,
    origem: m.origem || 'vendedor', validacao: body.status === 'vendida' ? 'aprovada' : (m.validacao || ''), lockOwner: '', lockExpires: ''
  });
...
function adminSalvarMesa(body) {
  ...
  saveMesa_(m, {
    status: body.status, comprador: body.comprador, contato: body.contato, vendedor: body.vendedor,
    pagamento: body.pagamento, comprovanteNome: comp.nome, comprovanteUrl: comp.url,
    origem: m.origem || 'admin', validacao: body.status === 'vendida' ? 'aprovada' : (body.status === 'livre' ? '' : m.validacao), lockOwner: '', lockExpires: ''
  });
